...
// if (config.debug) {
  if (config.chatProtocol.active === 1) {
    connection.xmlInput = function(data) { typeof data.children !== 'undefined' && console.log('[QBChat RECV]:', data.children[0]); };
    connection.xmlOutput = function(data) { typeof data.children !== 'undefined' && console.log('[QBChat SENT]:', data.children[0]); };
  } else {
