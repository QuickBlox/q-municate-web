...
// if (config.debug) {
  if (config.chatProtocol.active === 1) {
    connection.xmlInput = function(data) { data.children.length > 0 && console.log('[QBChat RECV]:', data.children[0]); };
    connection.xmlOutput = function(data) { data.children.length > 0 && console.log('[QBChat SENT]:', data.children[0]); };
  } else {
